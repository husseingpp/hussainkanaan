...
describe('computeSupply', () => {
  it('field units count toward supply weight', () => {
    ...
    const inf  = makeUnit('player', 'infantry', 100, 100);
    const tank = makeUnit('player', 'tank', 200, 100);
    const { fieldWeight, capacity } = computeSupply([inf, tank], [city], 'player');
    expect(fieldWeight).toBe(UNIT_TYPES.infantry.supplyWeight + UNIT_TYPES.tank.supplyWeight);
  ...
  it('units inside a friendly city radius cost no supply', () => {
    ...
    const u = makeUnit('player', 'infantry', 100, 100);
...
describe('starvation', () => {
  it('over-cap field units lose HP', () => {
    // 1 city → capacity 5; 6 infantry (supplyWeight=1 each → field weight 6).
    const city = makeCity(0, 'player', 2000, 2000);
    const units = Array.from({ length: 6 }, (_, i) => makeUnit('player', 'infantry', i * 10, 10));
  ...
  it('under-cap — no starvation', () => {
    // 2 cities → capacity 10; only 3 infantry (weight 3) in the field.
    const cities = [makeCity(0, 'player', 2000, 2000), makeCity(1, 'player', 2100, 2000)];
    const units = [
      makeUnit('player', 'infantry', 100, 100),
      makeUnit('player', 'infantry', 120, 100),
      makeUnit('player', 'infantry', 140, 100),
    ];
...
describe('capture', () => {
  ...
  it('capture resets when contested', () => {
    const city = makeCity(0, 'neutral', 200, 200);
    const player = makeUnit('player', 'infantry', 200, 200);
    const enemy  = makeUnit('enemy',  'infantry', 200, 200);
...
describe('production', () => {
  it('a queued infantry unit spawns after its spawnTime', () => {
    const city = makeCity(0, 'player', 200, 200);
    city.productionQueue.push({ kind: 'infantry', progress: 0, rallyPoint: null });
    const state = makeState([], [city]);
    const hash = new SpatialHash<Unit>();

    expect(state.units.length).toBe(0);
    stepEconomy(state, hash, UNIT_TYPES.infantry.spawnTime + 0.01);
    expect(state.units.length).toBe(1);
    expect(state.units[0]!.kind).toBe('infantry');
  ...
  it('spawned unit marches to the rally point', () => {
    ...
    city.productionQueue.push({ kind: 'infantry', progress: 0, rallyPoint: rally });
    const state = makeState([], [city]);
    const hash = new SpatialHash<Unit>();
  ...
  it('neutral cities cannot produce', () => {
    ...
    stepEconomy(state, hash, UNIT_TYPES.infantry.spawnTime + 1);
    expect(state.units.length).toBe(0);
  });
});
