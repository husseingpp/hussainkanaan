...
interface ProfileData {
  member: Member | null;
  terms: Term[];
  sponsorships: SponsorshipWithBill[];
  votes: VoteWithBill[];
...
export function MemberProfile() {
  const params = useParams<{ bioguideId: string }>();
    ...
    async () => {
      const [member, terms, sponsorships, votes, alignment, promises] =
        await Promise.all([
          ...
          getMemberSponsorships(bioguideId),
          getMemberVotes(bioguideId),
          getMemberAlignment(bioguideId),
          getMemberPromises(bioguideId),
        ]);
      return { member, terms, sponsorships, votes, alignment, promises };
    },
    [bioguideId],
    {
      member: null,
  ...
  if (loading) {
    return (
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8 text-sm text-gray-400">
        Loading member…
      </div>
    );
  }

  const { member, terms, sponsorships, votes, alignment, promises } = data;

  if (!member) {
    return (
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16 text-center">
        <p className="text-gray-500 font-medium">Member not found.</p>
        <Link
          href="/"
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 flex gap-6 items-start">
        ...
        <div className="min-w-0">
          ...
          {partyLabel && (
            <p className="mt-2 text-sm text-gray-500">Party: {partyLabel}</p>
          )}
      ...
      {/* Political alignment — wing is a bucket of the sourced DW-NOMINATE
          score, always shown with the raw number + its source. */}
