...
import type { Bill } from "../../../lib/database.types.ts";

interface BillData {
  bill: Bill | null;
  sponsorships: SponsorshipWithMember[];
}
...
export function BillDetail() {
  ...
  const { data, loading } = useAsync<BillData>(
    async () => {
      const bill = await getBill(id);
      const sponsorships = bill ? await getBillSponsorships(bill.id) : [];
      return { bill, sponsorships };
    },
    [id],
    { bill: null, sponsorships: [] },
  );
  ...
  const { bill, sponsorships } = data;

  if (!bill) {
    return (
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-16 text-center">
        <p className="text-gray-500 font-medium">Bill not found.</p>
        <Link
          href="/bills"
    ...
    <div className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
      ...
      <section className="mt-8">
        ...
        {primary.length > 0 ? (
          ...
          <p className="text-sm text-gray-400">No sponsor data available yet.</p>
        )}
      </section>
